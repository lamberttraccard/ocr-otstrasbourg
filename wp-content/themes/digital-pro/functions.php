...
function digital_scripts_styles() {
	...
	wp_enqueue_style( 'sweetalert', get_stylesheet_directory_uri() . '/css/sweetalert.css', array(), '1.0.0');
	...
	wp_enqueue_script( 'sweetalert', get_stylesheet_directory_uri() . '/js/sweetalert.min.js', array( 'jquery' ), '1.0.0', true );
	wp_enqueue_script( 'digital-main-script', get_stylesheet_directory_uri() . '/js/main.js', array( 'jquery' ), '1.0.0', true );
	$output = array(
...
function sp_custom_footer() {
	echo '<p>Copyright © 2016 Office de tourisme de Strasbourg et sa région</p>';
}
...
//* Modify the result for text
add_filter( 'genesis_search_title_text', 'digital_search_title_text' );
function digital_search_title_text() {
	return 'Résultats pour:';
}

//* Customize search form input button text
add_filter( 'genesis_search_button_text', 'digital_search_button_text' );
function digital_search_button_text( $text ) {
...
if( function_exists('acf_add_options_page') ) {
	...
	acf_add_options_page(array(
		...
		'menu_slug' 	=> 'activity_settings',
		'capability'	=> 'edit_posts',
		'redirect'		=> false
	));
}

// ADD content for activities pages
add_action('genesis_entry_content', 'add_content_for_activite_page');
function add_content_for_activite_page() {
	if(is_page('plus-infos')){
		get_template_part('/templates/content', 'activity-list');
	}
	if(is_page('activites-du-mois')){
		get_template_part('/templates/content', 'activity-table');
	}
}

// Group the activities by schedules and only display for current month
function activities_by_schedule($activities) {

	$activitiesBySchedule = array();
//	$currentMonth = date('m');
//	$currentYear = date('Y');
	...
	{
		foreach ($activity['schedules'] as $key2 => $schedule)
		{
			$month = explode('/',$schedule['schedule'])[1];
			$year = explode('/',$schedule['schedule'])[2];

			if ($month == $currentMonth && $year == $currentYear) {
				$activitiesBySchedule[$schedule['schedule']]['activite-' . ($key + 1)] = $activity;
			}
		}
	}

	// Sort the array by key
	ksort($activitiesBySchedule);

	return $activitiesBySchedule;
}
